...
// ============================================================
// STATIC FEED TOGGLE — run WITHOUT the Python server.
//   window.STATIC_FEED = "static/data/events.json"  -> replay that file
//                        on a loop through the normal pipeline (no
//                        WebSocket, works on any static host incl.
//                        GitHub Pages — serve with Range support for
//                        the basemap, see RUN.txt).
//   window.STATIC_FEED = null                        -> original behavior
//                        (WebSocket to the Python AttackMapServer).
// Feed file shape: { "stats": {...}, "events": [ <Traffic messages> ] }
// — the exact message format the WebSocket would carry; see
// static/data/events.json for a sample (src_*/dst_* lat+lon pairs).
// ============================================================
window.STATIC_FEED = "static/data/events.json";
window.STATIC_FEED_PACE_MS = 1200;   // delay between replayed events
window.STATIC_FEED_REFRESH_MS = 5 * 60000;  // re-fetch the feed file this often
                                            // (cron-regenerate the file => live map);
                                            // set to 0 to load once and just loop
...
function connectWebSocket() {
  ...
  if (window.STATIC_FEED) {
    // Mock socket: same object contract, fed from a JSON file. The rest of
    // this function attaches onopen/onmessage/onclose to it unchanged.
    webSocket = { readyState: 1, close() {}, send() {},
                  onopen: null, onmessage: null, onclose: null, onerror: null };
    window.webSocket = webSocket;
    const sock = webSocket;
    fetch(window.STATIC_FEED, { cache: "no-store" })
      .then(r => r.json())
      .then(feed => {
        const events = feed.events || feed || [];
        sock.onopen && sock.onopen();
        if (feed.stats) sock.onmessage && sock.onmessage({ data: JSON.stringify(feed.stats) });
        let i = 0;
        let live = events;
        if (window.STATIC_FEED_REFRESH_MS > 0) {
          setInterval(() => {
            fetch(window.STATIC_FEED, { cache: "no-store" })
              .then(r => r.json())
              .then(fresh => { live = fresh.events || fresh || live; })
              .catch(() => {});
          }, window.STATIC_FEED_REFRESH_MS);
        }
        (function step() {
          const events = live;
          ...
          i += 1;
          sock.onmessage && sock.onmessage({ data: JSON.stringify(ev) });
          window.lastWebSocketMessageTime = Date.now();
          window.lastValidDataTime = Date.now();
          setTimeout(step, window.STATIC_FEED_PACE_MS || 1200);
        })();
      })
      .catch(err => console.error("[STATIC_FEED] failed to load:", err));
  } else {
